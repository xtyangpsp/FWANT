...
%smot_list=[64];
smot_list=[2 4 8 12 16 24];
damp_list=[2 4 8 12 16 24];
zval_list=[4 7 18 32 51 74 97 134 212];
zindx_list=[51 50 47 44 42 40 38 35 29];
...
minlon=-167.5; maxlon=-129.5; minlat=53.5;maxlat=67.5;
%topo=qt;
%Zin=topo;
...
%     plot(sxlon,sylat, '*k')
    ...
    fnm_fig=['figure_Vp_s' num2str(smot_list(nsmot)) 'd' num2str(damp_list(ndamp)) '_z' KDNM];
    fnm_outfile=['Vp_' int2str(nk) '.txt'];
    Vmax=3;
    end;
    if strcmp(KCMPNM, 'Vs')
    fnm_fig=['figure_Vs_s' num2str(smot_list(nsmot)) 'd' num2str(damp_list(ndamp)) '_z' KDNM];
...
hid=figure('Position',[400 400 700 700]); hold on, box on, axis on
...
%contour(lontopo,lattopo,topo,[1 1],'Color','k');
...
% [clon,clat]=textread('/net/fs01/data/haiying/Proj/cascadia/misc/PNWcoast_revised.dat','%f %f');
% plot(clon+360,clat,'k.','MarkerSize',2);
% 
% state=[];
% load /net/fs01/data/haiying/Proj/cascadia/misc/us_states.mat
