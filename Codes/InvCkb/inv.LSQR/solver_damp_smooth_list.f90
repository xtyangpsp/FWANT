...
! Replace the current SP declarations with this:
#ifdef USE_DP
...
integer :: n_damp, n_smooth, id, is, i
integer(I8B) :: damp_start_idx, smooth_start_idx
real(DP), allocatable :: damp_list(:), smooth_list(:)
character(len=1000) :: line_buffer
character(len=132) :: fnm_out_grid
...
! --- Read Damping List ---
write(*,*) "Enter the damping values (space-separated):"
read(*, '(A)') line_buffer
n_damp = count_values(line_buffer)
allocate(damp_list(n_damp))
read(line_buffer, *) damp_list

! --- Read Smoothing List ---
write(*,*) "Enter the smoothing values (space-separated):"
read(*, '(A)') line_buffer
n_smooth = count_values(line_buffer)
allocate(smooth_list(n_smooth))
read(line_buffer, *) smooth_list
...
contains
!----------------------------------------------------------
subroutine update_weights(d_start, s_start, d_val, s_val)
    integer(I8B), intent(in) :: d_start, s_start
    real(DP), intent(in) :: d_val, s_val
    integer(I8B) :: k

    ! Update Damping coefficients (usually 1.0 scaled by d_val)
    do k = d_start, s_start - 1
        ra(k) = d_val 
    end do

    ! Update Smoothing coefficients (assuming they were 1.0 scaled by s_val)
    ! Note: If your smoothing op has internal weights (like -1, 2, -1), 
    ! you'll need to store the 'raw' op and multiply here.
    do k = s_start, nel
        ra(k) = ra_raw(k) * s_val 
    end do
end subroutine
!
function count_values(str) result(cnt)
