...
real(DP) :: rhs,weig_dat
real(DP) :: rhs_min,rhs_max,rhs_min_weig,rhs_max_weig
...
public :: yang_main_init, yang_worker_init, &
          yang_damping, yang_smoothing, yang_solving, yang_output, &
          read_gmat, error_except, &
          fnm_list, fnm_smooth,  num_cmp, nsta, nevt, nloc, weig_sta, weig_evt, weig_loc, &
          lamb, eta, ker_thres, data_thres

!----------------------------------------------------------
contains
!----------------------------------------------------------

    subroutine yang_main_init()
    ...
    subroutine yang_worker_init(rank, nlines, nworkers)
        integer, intent(in) :: rank, nlines, nworkers

        call init_smoothing(rank, fnm_smooth, num_cmp,&
            nrow_smoth,nbd_smoth,nel_smoth)

        call init_data_damp(rank, nlines, nworkers, fnm_list,num_cmp,nsta,nevt,nloc,  &
    ...
    subroutine read_gmat(rank, nlines, nworkers)
        !----------------------------------------------------------------------
        ! N. Trupin 2023.11.01
        integer, intent(in) :: rank, nlines, nworkers

        listid = 10000 + 2 * rank; gid = 10000 + 2 * rank + 1
        rhs_min = 1.0e7; rhs_max = -1.e7
        rhs_min_weig = 1.e7; rhs_max_weig = -1.e7

        open(listid, file = trim(fnm_list), status = "old", iostat = ierr)
        if (ierr /= 0) &
            call error_except("fnm_list open err: "//trim(fnm_list))

        icount = 0

        ! skip to worker location
        do
            read(listid, *, iostat = ierr)
            if (ierr /= 0 .or. (icount .ge. (nlines / nworkers) * (rank - 1)+ 1)) exit
            icount = icount + 1
        end do
        ...
        list_loop: do
            ...
            if (ierr /= 0 .or. (icount .ge. (nlines / nworkers) + 1)) exit
            ...
            do k=1,num_cmp
                ...
                if (ierr>0) &
                    call error_except("open kernel err:"//trim(fnm_G))
            ...
            n=index(rcdstr(1:len_trim(rcdstr))," ");rcdstr(1:)=rcdstr(n+1:);
    ...
    end subroutine yang_smoothing

    subroutine yang_solving()
        write(*,*) "    begin solving"
        call splsqr(nit)
    ...
    subroutine yang_output()
        write(*,*) "    exporting ..."
        call export_result
        write(*,*) "    evaluating (writing summary) ..."
...
subroutine init_smoothing(rank, filenm,ncmp,max_row,max_bd,max_nel)
...
subroutine init_data_damp(rank, nlines, nworkers, filenm,ncmp,nst,nev,nlo,  &
...
fid=5000 + 2 * rank; gid=5000 + 2 * rank + 1
max_st=0
max_ev=0
max_lo=0
max_dat=0
...
real(DP) :: fit,dvar,fit_weig,dvar_weig,fit_kai2
real(DP) :: mean,vmin,vmax,cov,norm,smocov
real(DP) :: vmin_sta,vmax_sta,vmin_evt,vmax_evt,vmin_loc,vmax_loc
real(DP) :: tmp,tmp_weig
real(DP) :: rhs,rhs_weig
integer :: i,j,k,n,m,nblk,ierr,fid,gid
integer(I8B)::ndat,nael
real(SP) :: ker0

!------------- data variance reduction ------------------
fit=0.0_DP; dvar=0.0_DP
...
write(*,*) "damping, smoothing, data_var_reduc, model_norm"
write(*,*) lamb, eta, fit, norm
write(*,*) ""
