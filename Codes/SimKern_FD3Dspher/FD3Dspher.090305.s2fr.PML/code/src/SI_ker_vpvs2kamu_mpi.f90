...
! read kernel conf
fnm_ker_conf='TomoKernel_conv.conf'
...
call swmpi_datatype
  ...
  call MPI_BCAST(indir,SEIS_STRLEN,MPI_CHARACTER,0,SWMPI_COMM,ierr)
...
do npk=1,picknum(ncmp)
   ...
   !   infnm_ker=get_fnm_snapnode_n('./','kernel_lamu_',kid,0,n_i,n_j,n_k)
   !   call nfseis_grid3d_def                                             &
   !       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),        &
   !       snap_subc(1,kid),snap_subc(2,kid), snap_subc(3,kid),in_ncid(npk), &
   !       "Finite frequency kernel")
   !   call nfseis_grid3d_defvar(in_ncid(npk),'phase_la'    ,kapid(npk))
   !   call nfseis_grid3d_defvar(in_ncid(npk),'amplitude_la',kaqid(npk))
   !   call nfseis_grid3d_defvar(in_ncid(npk),'phase_mu'    ,kbpid(npk))
   !   call nfseis_grid3d_defvar(in_ncid(npk),'amplitude_mu',kbqid(npk))
   !   call nfseis_grid3d_enddef(in_ncid(npk))
   !else
   !   infnm_ker=get_fnm_snapnode_n('./','kernel_lamu_',kid,0,n_i,n_j,n_k)
   !   call nfseis_open(trim(pnm_ker(npk))//"/"//trim(infnm_ker),in_ncid(npk))
   !   call nfseis_inq_varid(in_ncid(npk),'phase_la'    ,kapid(npk))
   !   call nfseis_inq_varid(in_ncid(npk),'amplitude_la',kaqid(npk))
   !   call nfseis_inq_varid(in_ncid(npk),'phase_mu'    ,kbpid(npk))
   !   call nfseis_inq_varid(in_ncid(npk),'amplitude_mu',kbqid(npk))
   !end if
   ! output kernel nc file
   if (flag_fst .and. (blki==1 .and. blkj==1 .and. blkk==1)) then
      oufnm_ker=get_fnm_snapnode_n('./','kernel_kamu_',kid,0,n_i,n_j,n_k)
      call nfseis_grid3d_def(trim(pnm_ker(npk))//"/"//trim(oufnm_ker),   &
         snap_subc(1,kid),snap_subc(2,kid), snap_subc(3,kid),ncid(npk),  &
          "Finite frequency kernel")
      call nfseis_grid3d_defvar(ncid(npk),'phase_ka'    ,kapid(npk))
      call nfseis_grid3d_defvar(ncid(npk),'amplitude_ka',kaqid(npk))
      call nfseis_grid3d_defvar(ncid(npk),'phase_mu'    ,kbpid(npk))
      call nfseis_grid3d_defvar(ncid(npk),'amplitude_mu',kbqid(npk))
      call nfseis_grid3d_enddef(ncid(npk))
   else
      oufnm_ker=get_fnm_snapnode_n('./','kernel_kamu_',kid,0,n_i,n_j,n_k)
      call nfseis_open(trim(pnm_ker(npk))//"/"//trim(oufnm_ker),ncid(npk))
      call nfseis_inq_varid(ncid(npk),'phase_ka'    ,kapid(npk))
      call nfseis_inq_varid(ncid(npk),'amplitude_ka',kaqid(npk))
      call nfseis_inq_varid(ncid(npk),'phase_mu'    ,kbpid(npk))
      call nfseis_inq_varid(ncid(npk),'amplitude_mu',kbqid(npk))
   end if
end do
...
do npk=1,picknum(ncmp)
   ...
   !call scatter2kernel(VV,UU,Uda,Udb,nt1,nt2,kdt,bsubc,Kap,Kaq,Kbp,Kbq)

   ! input kernel nc file
   infnm_ker=get_fnm_snapnode_n('./','kernel_',kid,0,n_i,n_j,n_k)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'phase_Vp',    &
       kap,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'amplitude_Vp',&
       kaq,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'phase_Vs',    &
       kbp,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'amplitude_Vs',&
       kbq,bsubs,bsubc,bsubt)
   ! put
   do kk=1,bsubc(3)
   do jj=1,bsubc(2)
   do ii=1,bsubc(1)
      Kap(ii,jj,kk)=Kap(ii,jj,kk)/(2.0_SP*(lambda(ii,jj,kk)+2.0_SP*mu(ii,jj,kk))) 
      Kbp(ii,jj,kk)=Kbp(ii,jj,kk)/mu(ii,jj,kk)/4.0_SP+Kap(ii,jj,kk)
      Kbp(ii,jj,kk)=(2.0_SP*Kbp(ii,jj,kk)-2.0_SP*Kap(ii,jj,kk)/3.0_SP)*mu(ii,jj,kk)
      Kap(ii,jj,kk)=Kap(ii,jj,kk)*(lambda(ii,jj,kk)+2.0_SP*mu(ii,jj,kk)/3.0_SP)

      Kaq(ii,jj,kk)=Kaq(ii,jj,kk)/(2.0_SP*(lambda(ii,jj,kk)+2.0_SP*mu(ii,jj,kk))) 
      Kbq(ii,jj,kk)=Kbq(ii,jj,kk)/mu(ii,jj,kk)/4.0_SP+Kaq(ii,jj,kk)
      Kbq(ii,jj,kk)=(2.0_SP*Kbq(ii,jj,kk)-2.0_SP*Kaq(ii,jj,kk)/3.0_SP)*mu(ii,jj,kk)
      Kaq(ii,jj,kk)=Kaq(ii,jj,kk)*(lambda(ii,jj,kk)+2.0_SP*mu(ii,jj,kk)/3.0_SP)
      !Kbq(ii,jj,kk)=2.0_SP*(Kbq(ii,jj,kk)-Kaq(ii,jj,kk))*2.0_SP*mu(ii,jj,kk)
      !Kaq(ii,jj,kk)=Kaq(ii,jj,kk)*2.0_SP*(lambda(ii,jj,kk)+2.0_SP*mu(ii,jj,kk))
   end do
   end do
   end do
...
do m=1,n-1
   ...
   ExxS(i,j,k,n)=x1*lam(i,j,k)
   ExyS(i,j,k,n)=2.0_SP*x2*miu(i,j,k)
end do
end if
...
do m=1,n-1
   ...
   EyyS(i,j,k,n)=x1*lam(i,j,k)
   EyzS(i,j,k,n)=2.0_SP*x2*miu(i,j,k)
end do
end if
...
do m=1,n-1
   ...
   EzzS(i,j,k,n)=x1*lam(i,j,k)
   ExzS(i,j,k,n)=2.0_SP*x2*miu(i,j,k)
end do
end if
