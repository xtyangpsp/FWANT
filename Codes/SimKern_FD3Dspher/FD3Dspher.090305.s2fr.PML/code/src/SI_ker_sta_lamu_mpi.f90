...
do npk=1,picknum(ncmp)
   ...
   if (flag_fst .and. (blki==1 .and. blkj==1 .and. blkk==1)) then
      fnm_ker=get_fnm_snapnode_n('./','kernel_lamu_',kid,0,n_i,n_j,n_k)
      ...
      call nfseis_grid3d_defvar(ncid(npk),'phase_la'    ,kapid(npk))
      call nfseis_grid3d_defvar(ncid(npk),'amplitude_la',kaqid(npk))
      ...
      call nfseis_inq_varid(ncid(npk),'phase_la'    ,kapid(npk))
      call nfseis_inq_varid(ncid(npk),'amplitude_la',kaqid(npk))
