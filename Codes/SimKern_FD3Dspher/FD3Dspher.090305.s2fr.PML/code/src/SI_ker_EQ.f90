...
evt_loop: do
  ...
  write(*,*)'flag_udxyz',flag_udx,flag_udy,flag_udz
  ...
  !Vz = eoshift(Vz, shift=-9)
  !Uz = eoshift(Uz, shift=-9)
...
if (snap_tcnt(kid)<knt) then
  ...
  ! Apply a time shift to the stress
  !  For an earthquake source
  !  T_eq(i,j,k,t)  ::= T ( EQ_STF(t=0) * G )
  !  T_sgt(i,j,k,t) ::= T ( F(t=t_sgt)  * G )  ; t_sgf = 22.5, dt = 2.5, shift = -9 pts 
  !     negative values shift to the right, positive in time  ---> way
 
  write(*,*)'SHIFT SGT'
  ExxZ = eoshift(ExxZ,shift=9, dim=4)
  EyyZ = eoshift(EyyZ,shift=9, dim=4)
  EzzZ = eoshift(EzzZ,shift=9, dim=4)
  ExyZ = eoshift(ExyZ,shift=9, dim=4)
  ExzZ = eoshift(ExzZ,shift=9, dim=4)
  EyzZ = eoshift(EyzZ,shift=9, dim=4)

  flag_sgtz=.true.
  call stress2strain(ExxZ,EyyZ,EzzZ,ExyZ,ExzZ,EyzZ,lambda,mu,bsubc)
end if
! load event sgt
write(*,*)'load event SGT'
if (snap_tcnt(kid)<knt) then
   m=0; n=0; mt=snap_tcnt(kid)
   do 
      write(*,*)'loading SGT m:',m,' section'
      if (m>=knt) exit
      n=n+1
      filenm=get_fnm_snapnode_n(                                    &
  ...
  !write(*,*)'SHIFT SGT'
  !ExxS = eoshift(ExxS,shift=-9, dim=4)
  !EyyS = eoshift(EyyS,shift=-9, dim=4)
  !EzzS = eoshift(EzzS,shift=-9, dim=4)
  !ExyS = eoshift(ExyS,shift=-9, dim=4)
  !ExzS = eoshift(ExzS,shift=-9, dim=4)
  !EyzS = eoshift(EyzS,shift=-9, dim=4)
