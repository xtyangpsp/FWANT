...
do npk=1,picknum(ncmp)
   ...
   do n=1,nthd
      ...
      fnm_ker=get_fnm_snapnode_n('./','kernel_kamu_',kid,0,n_i,n_j,n_k)
      fnm_ker=trim(pnm_ker)//"/"//trim(fnm_ker)
#ifdef PhaseDelay
      call nfseis_varget(trim(fnm_ker),'phase_ka',Kap(i1:i2,j1:j2,k1:k2),bsubs,bsubc,bsubt)
      call nfseis_varget(trim(fnm_ker),'phase_mu',Kbp(i1:i2,j1:j2,k1:k2),bsubs,bsubc,bsubt)
#endif
#ifdef AmplitudeReduction
      call nfseis_varget(trim(fnm_ker),'amplitude_ka',Kaq(i1:i2,j1:j2,k1:k2),bsubs,bsubc,bsubt)
      call nfseis_varget(trim(fnm_ker),'amplitude_mu',Kbq(i1:i2,j1:j2,k1:k2),bsubs,bsubc,bsubt)
#endif
   end do
