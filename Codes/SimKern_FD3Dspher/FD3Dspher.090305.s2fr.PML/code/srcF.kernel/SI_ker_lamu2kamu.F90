...
integer :: i,i3,j,k,m,n,mt,ierr,i1,i2,j1,j2,k1,k2
integer :: ii,jj,kk
integer :: p(1)
integer,dimension(SEIS_GEO) :: &
   bsubs,bsubc,bsubt,          &
...
do npk=1,picknum(ncmp)
   ...
   if (flag_fst .and. (blki==1 .and. blkj==1 .and. blkk==1)) then
      oufnm_ker=get_fnm_snapnode_n('./','kernel_',kid,0,n_i,n_j,n_k)
...
if (myid==0) &
   ...
   infnm_ker=get_fnm_snapnode_n('./','kernel_lamu_',kid,0,n_i,n_j,n_k)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'phase_la',    &
       kap,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'amplitude_la',&
       kaq,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'phase_mu',    &
       kbp,bsubs,bsubc,bsubt)
   call nfseis_varget                                                        &
       (trim(indir)//trim(pnm_ker(npk))//"/"//trim(infnm_ker),'amplitude_mu',&
       kbq,bsubs,bsubc,bsubt)
   ! put
   do kk=1,bsubc(3)
   do jj=1,bsubc(2)
   do ii=1,bsubc(1)
      Kap(ii,jj,kk)=Kap(ii,jj,kk)/lambda(ii,jj,kk) 
      Kbp(ii,jj,kk)=Kbp(ii,jj,kk)/mu(ii,jj,kk)/2.0_SP
      ...
      Kaq(ii,jj,kk)=Kaq(ii,jj,kk)/lambda(ii,jj,kk) 
      Kbq(ii,jj,kk)=Kbq(ii,jj,kk)/mu(ii,jj,kk)/2.0_SP 
