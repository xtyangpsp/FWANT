program SI_ker_sta_rho2

! rho kernel with lamda and mu as inversion variables
...
real(SP),dimension(:,:,:),allocatable :: Kap,Kaq,Kbp,Kbq,Kcp,Kcq
...
integer :: VxSid,VySid,VzSid
integer :: VxFid,VyFid,VzFid

integer,dimension(:),allocatable :: ncid,ncid_rho
integer,dimension(:),allocatable :: kapid,kaqid,kbpid,kbqid,kcpid,kcqid
...
!----------------------------------------------------------------------
kernode_if : if (snap_ishere(kid) .and. vel_out(kid)) then
...
if ( product(bsubc)*knt/=size(VxS) ) then
...
if ( flag_udx .and. (.not. flag_sgtx) ) then
   if ((.not. allocated(VxX)) .or. (product(bsubc)*knt/=size(VxX))) then
      call alloc_velx(bsubc(1),bsubc(2),bsubc(3),knt)
   end if
if (snap_tcnt(kid)<knt) then
   ...
   do 
      ...
      call nfseis_inq_varid(ncidF, 'Vx', VxFid)
      call nfseis_inq_varid(ncidF, 'Vy', VyFid)
      call nfseis_inq_varid(ncidF, 'Vz', VzFid)
      ...
      ierr=nf90_get_var(ncidF,VxFid,VxX(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VyFid,VyX(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VzFid,VzX(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      if (ierr /= nf90_noerr) &
   ...
   ierr=nf90_get_var(ncidF,VxFid,VxX,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VyFid,VyX,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VzFid,VzX,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   call nfseis_close(ncidF)
end if
  flag_sgtx=.true.
...
if ( flag_udy .and. (.not. flag_sgty) ) then
   if ((.not. allocated(VxY)) .or. (product(bsubc)*knt/=size(VxY))) then
      call alloc_vely(bsubc(1),bsubc(2),bsubc(3),knt)
   end if
if (snap_tcnt(kid)<knt) then
   m=0; n=0; mt=snap_tcnt(kid)
   do 
      ...
      ierr=nf90_get_var(ncidF,VxFid,VxY(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VyFid,VyY(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VzFid,VzY(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      if (ierr /= nf90_noerr) &
      call nfseis_except(ierr,'tomo_kernel: read sgtx fail')
      m=m+mt
      call nfseis_close(ncidF)
   ...
   ierr=nf90_get_var(ncidF,VxFid,VxY,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VyFid,VyY,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VzFid,VzY,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   call nfseis_close(ncidF)
end if
  flag_sgty=.true.
...
if ( flag_udz .and. (.not. flag_sgtz) ) then
   if ((.not. allocated(VxZ)) .or. (product(bsubc)*knt/=size(VxZ))) then
      call alloc_velz(bsubc(1),bsubc(2),bsubc(3),knt)
   end if
if (snap_tcnt(kid)<knt) then
   m=0; n=0; mt=snap_tcnt(kid)
   do 
      ...
      ierr=nf90_get_var(ncidF,VxFid,VxZ(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VyFid,VyZ(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidF,VzFid,VzZ(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      if (ierr /= nf90_noerr) &
      call nfseis_except(ierr,'tomo_kernel: read sgtx fail')
      m=m+mt
      call nfseis_close(ncidF)
   ...
   ierr=nf90_get_var(ncidF,VxFid,VxZ,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VyFid,VyZ,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidF,VzFid,VzZ,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   call nfseis_close(ncidF)
end if
  flag_sgtz=.true.
...
if (snap_tcnt(kid)<knt) then
   ...
   do 
      ...
      call nfseis_inq_varid(ncidS, 'Vx', VxSid)
      call nfseis_inq_varid(ncidS, 'Vy', VySid)
      call nfseis_inq_varid(ncidS, 'Vz', VzSid)
      if (m+mt>knt) mt=knt-m
      ...
      ierr=nf90_get_var(ncidS,VxSid,VxS(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidS,VySid,VyS(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      ierr=nf90_get_var(ncidS,VzSid,VzS(:,:,:,m+1:m+mt),(/bsubs,1/),(/bsubc,mt/),(/bsubt,1/))
      if (ierr /= nf90_noerr) &
   ...
   ierr=nf90_get_var(ncidS,VxSid,VxS,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidS,VySid,VyS,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   ierr=nf90_get_var(ncidS,VzSid,VzS,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   call nfseis_close(ncidS)
end if
...
do ncmp=1,6
   ...
   if (ncmp==4 .and. any(picknum(4:6)>0)) then
      ...
      call scatter_rotate(VxS,VyS,VzS,rot_mat,1,knt,bsubc)
   end if
...
do npk=1,picknum(ncmp)
   ...
   if (flag_fst .and. (blki==1 .and. blkj==1 .and. blkk==1)) then
      fnm_ker_rho=get_fnm_snapnode_n('./','kernel_rho2_',kid,0,n_i,n_j,n_k)
      call nfseis_grid3d_def(trim(pnm_ker(npk))//"/"//trim(fnm_ker_rho),   &
          snap_subc(1,kid),snap_subc(2,kid), snap_subc(3,kid),ncid_rho(npk), &
          "Finite frequency kernel")
      call nfseis_grid3d_defvar(ncid_rho(npk),'phase_rho'    ,kcpid(npk))
      call nfseis_grid3d_defvar(ncid_rho(npk),'amplitude_rho',kcqid(npk))
      ...
      fnm_ker_rho=get_fnm_snapnode_n('./','kernel_rho2_',kid,0,n_i,n_j,n_k)
      call nfseis_open(trim(pnm_ker(npk))//"/"//trim(fnm_ker_rho),ncid_rho(npk))
      call nfseis_inq_varid(ncid_rho(npk),'phase_rho'    ,kcpid(npk))
      call nfseis_inq_varid(ncid_rho(npk),'amplitude_rho',kcqid(npk))
...
if (myid==0) &
   ...
   call scatter2kernel_rho(VV,UU,Uda,nt1,nt2,kdt,bsubc,Kcp,Kcq)

   ! put
   !ierr=nf90_get_var(ncid(npk),kapid(npk),Kap,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   !ierr=nf90_get_var(ncid(npk),kaqid(npk),Kaq,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   !ierr=nf90_get_var(ncid(npk),kbpid(npk),Kbp,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))
   !ierr=nf90_get_var(ncid(npk),kbqid(npk),Kbq,(/bsubs,1/),(/bsubc,knt/),(/bsubt,1/))

   !Kcp=Kcp+(Kap+Kbp)/2.0
   !Kcq=Kcq+(Kaq+Kbq)/2.0
   call nfseis_put(ncid_rho(npk),kcpid(npk),Kcp,bsubs,bsubc,bsubt)
   call nfseis_put(ncid_rho(npk),kcqid(npk),Kcq,bsubs,bsubc,bsubt)
end do ! npk

! close nc file
do npk=1,picknum(ncmp)
   call nfseis_close(ncid(npk))
   call nfseis_close(ncid_rho(npk))
end do ! npk
...
subroutine alloc_pick(npk)
  ...
  if (allocated(ncid_rho ))    deallocate(ncid_rho )
  ...
  if (allocated(kcpid))    deallocate(kcpid)
  if (allocated(kcqid))    deallocate(kcqid)
...
subroutine alloc_ker_var(ki,kj,kk,kt)
  ...
  allocate(VxS(ki,kj,kk,kt)); VxS=0.0
  allocate(VyS(ki,kj,kk,kt)); VyS=0.0
  allocate(VzS(ki,kj,kk,kt)); VzS=0.0
  ...
  allocate(Kcp(ki,kj,kk   )); Kcp=0.0
  allocate(Kcq(ki,kj,kk   )); Kcq=0.0
...
subroutine realloc_ker_var(ki,kj,kk,kt)
...
#endif
  if (allocated(VxS)) deallocate(VxS)
  if (allocated(VyS)) deallocate(VyS)
  if (allocated(VzS)) deallocate(VzS)
  ...
  if (allocated(Kcp)) deallocate(Kcp)
  if (allocated(Kcq)) deallocate(Kcq)
...
subroutine alloc_velx(ki,kj,kk,kt)
  integer,intent(in) :: ki,kj,kk,kt
#ifdef VERBOSE
  write(*,*) idstr,' alloc_velx'
#endif
  if (allocated(VxX)) deallocate(VxX)
  if (allocated(VyX)) deallocate(VyX)
  if (allocated(VzX)) deallocate(VzX)
  allocate(VxX(ki,kj,kk,kt)); VxX=0.0
  allocate(VyX(ki,kj,kk,kt)); VyX=0.0
  allocate(VzX(ki,kj,kk,kt)); VzX=0.0
end subroutine alloc_velx
...
subroutine alloc_vely(ki,kj,kk,kt)
  integer,intent(in) :: ki,kj,kk,kt
#ifdef VERBOSE
  write(*,*) idstr,' alloc_vely'
#endif
  if (allocated(VxY)) deallocate(VxY)
  if (allocated(VyY)) deallocate(VyY)
  if (allocated(VzY)) deallocate(VzY)
  allocate(VxY(ki,kj,kk,kt)); VxY=0.0
  allocate(VyY(ki,kj,kk,kt)); VyY=0.0
  allocate(VzY(ki,kj,kk,kt)); VzY=0.0
end subroutine alloc_vely
...
subroutine alloc_velz(ki,kj,kk,kt)
  integer,intent(in) :: ki,kj,kk,kt
#ifdef VERBOSE
  write(*,*) idstr,' alloc_velz'
#endif
  if (allocated(VxZ)) deallocate(VxZ)
  if (allocated(VyZ)) deallocate(VyZ)
  if (allocated(VzZ)) deallocate(VzZ)
  allocate(VxZ(ki,kj,kk,kt)); VxZ=0.0
  allocate(VyZ(ki,kj,kk,kt)); VyZ=0.0
  allocate(VzZ(ki,kj,kk,kt)); VzZ=0.0
end subroutine alloc_velz

subroutine alloc_media_local(ki,kj,kk)
  integer,intent(in) :: ki,kj,kk
  if (allocated(rho)) deallocate(rho)
  allocate(rho(ki,kj,kk)); rho=0.0
...
real(SP),dimension(:,:,:,:),intent(in) ::    &
    ...
    VxY,VyY,VzY,                          &
    VxZ,VyZ,VzZ
real(SP),dimension(:,:,:),intent(in) :: rh
!real(SP),dimension(:,:,:,:),intent(inout) :: &
!    ExxS,EyyS,EzzS,ExyS,ExzS,EyzS
!real(SP),dimension(:,:,:,:),intent(in) ::    &
!    ExxX,EyyX,EzzX,ExyX,ExzX,EyzX,           &
...
do j=1,cblk(2)
do i=1,cblk(1)
   E11S(1:nt2)=VxS(i,j,k,1:nt2);E22S=VyS(i,j,k,1:nt2);E33S=VzS(i,j,k,1:nt2)

! Ux
if (flag_udx) then
   E11R(1:nt2)=VxX(i,j,k,1:nt2);E22R=VyX(i,j,k,1:nt2);E33R=VzX(i,j,k,1:nt2)
do n=nt1,nt2
   x1=0.0_SP; x2=0.0_SP
do m=1,n-1
   x1=x1+1.0_SP*dt*(E11R(m)+E22R(m)+E33R(m))*(E11S(n-m)+E22S(n-m)+E33S(n-m))
end do
   VxS(i,j,k,n)=x1*1.0_SP*rh(i,j,k)
end do
end if

! Uy
if (flag_udy) then
   E11R(1:nt2)=VxY(i,j,k,1:nt2);E22R=VyY(i,j,k,1:nt2);E33R=VzY(i,j,k,1:nt2)
do n=nt1,nt2
   x1=0.0_SP; x2=0.0_SP
do m=1,n-1
   x1=x1+1.0_SP*dt*(E11R(m)+E22R(m)+E33R(m))*(E11S(n-m)+E22S(n-m)+E33S(n-m))
end do
   VyS(i,j,k,n)=x1*1.0_SP*rh(i,j,k)
end do
end if

! Uz
if (flag_udz) then
   E11R(1:nt2)=VxZ(i,j,k,1:nt2);E22R=VyZ(i,j,k,1:nt2);E33R=VzZ(i,j,k,1:nt2)
do n=nt1,nt2
   x1=0.0_SP; x2=0.0_SP
do m=1,n-1
   x1=x1+1.0_SP*dt*(E11R(m)+E22R(m)+E33R(m))*(E11S(n-m)+E22S(n-m)+E33S(n-m))
end do
   VzS(i,j,k,n)=x1*1.0_SP*rh(i,j,k)
end do
end if
...
subroutine scatter2kernel_rho(V,U,Sa, &
           nt1,nt2,dt,cblk,Kcp,Kcq)

real(SP),dimension(:),intent(in) :: V,U
real(SP),dimension(:,:,:,:),intent(in) :: Sa
integer,dimension(SEIS_GEO),intent(in) :: cblk
integer,intent(in) :: nt1,nt2
real(SP),intent(in) :: dt

real(SP),dimension(:,:,:),intent(out) :: Kcp,Kcq
...
do i=1,cblk(1)
   ...
   ! Forming kernels for tau_p and tau_q.
   Kcp(i,j,k)=0.5*dt*(V(nt1)*Ka(nt1)+V(nt2)*Ka(nt2))
   Kcq(i,j,k)=0.5*dt*(U(nt1)*Ka(nt1)+U(nt2)*Ka(nt2))
   do n=nt1+1,nt2-1
      Kcp(i,j,k)=Kcp(i,j,k)+V(n)*Ka(n)*dt
      Kcq(i,j,k)=Kcq(i,j,k)+U(n)*Ka(n)*dt
   end do
   
   Kcp(i,j,k)=Kcp(i,j,k)/V0
   Kcq(i,j,k)=Kcq(i,j,k)/U0

end do
end do
end do
end subroutine scatter2kernel_rho
