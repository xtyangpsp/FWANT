...
mx=76; my=50; mz=40;
fnm_blk=['../ite_0.025deg_03/inv.structure.kerVpVs/block.76x50x40.1x1x1.1x1x1.nc'];
...
fnm_try=['../ite_0.025deg_03/inv.structure.kerVpVs/Freq12345.Z.model/result.1th/try.damp8.smot8.st0.ev0.lo0.dat'];
...
itp_vp=FP(XYZ);itp_vpnan=isnan(itp_vp);
INDNaN=find(itp_vpnan==1);itp_vp(INDNaN)=0;

itp_vs=FS(XYZ);itp_vsnan=isnan(itp_vs);
clear INDNaN; INDNaN=find(itp_vsnan==1);itp_vs(INDNaN)=0;
...
for k=0:MPIK-1
 ...
 if i<10   
 fnm_out=['./updated_input_' ite_nm '/media_mpi0',num2str(i),'0',num2str(j),'0',num2str(k),'.nc']
 else
 fnm_out=['./updated_input_' ite_nm '/media_mpi',num2str(i),'0',num2str(j),'0',num2str(k),'.nc']
 end
