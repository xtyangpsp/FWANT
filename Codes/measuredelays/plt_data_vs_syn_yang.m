function plt_data_vs_syn_yang(datadir,syndir,evname, fband, subplotpar,my_ylim)
...
system(['mkdir ' char(evname), '_minsnr',  num2str(snrmin)]);
for k=1:nfb
    clear idx
    idx = find(mysnr(k,:)>=snrmin);
    %idx=find(dist0>=400 & azim>=225 & azim<=270);
...
%         plot(tt,scale*egffb_pos(:,i,k)+dist0(i),'k'); 
        ...
        fidoutegf=fopen([char(evname), '_minsnr',  num2str(snrmin),'/',char(evname), '_to_',stanames{i,1},'_obs_',num2str(round(pband(k,1))), ...
            '-', num2str(round(pband(k,2))) 's.txt'],'w');
        for pp=1:length(tt)
           fprintf(fidoutegf,'%g  %g\n',tt(pp), egffb_pos(pp,i,k));
        end
        fclose(fidoutegf);
        fidoutsyn=fopen([char(evname), '_minsnr',  num2str(snrmin),'/',char(evname), '_to_',stanames{i,1},'_syn_',num2str(round(pband(k,1))), ...
            '-', num2str(round(pband(k,2))) 's.txt'],'w');
        for pp=1:length(tt3)
           fprintf(fidoutsyn,'%g  %g\n',tt3(pp), synfbdata(pp,i,k));
        end
        fclose(fidoutsyn);
